...
const ARPUWorkflowOutputSchema = z.object({
...
// Step 1: Fetch subscriptions from Stripe MCP server (reused from MRR workflow)
...
// Step 2: Calculate ARPU using our calculation tool
...
export const arpuCalculationWorkflow = createWorkflow({
  id: 'arpu-calculation-workflow',
  description: 'Fetch Stripe subscription data and calculate Average Revenue Per User (ARPU)',
  inputSchema: ARPUWorkflowInputSchema,
  outputSchema: ARPUWorkflowOutputSchema,
  steps: [fetchSubscriptionsStep, calculateARPUStep],
})
